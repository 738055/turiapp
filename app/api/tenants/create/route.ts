...
export async function POST(req: NextRequest) {
  ...
  if (existing) {
    return NextResponse.json({ error: "Esse nome de loja ja esta em uso. Escolha outro." }, { status: 409 });
  }
  ...
  const templatePages = materializeStoreTemplatePages(selectedTemplate, {
    companyName: d.company_name,
    whatsapp: d.whatsapp_number,
  });
  const { data: createdPages } = await serviceClient
    .from("pages")
    .insert(
  ...
  if (createdPages?.length) {
    const pageBySlug = new Map(createdPages.map((page) => [page.slug, page.id]));
    ...
    if (sections.length) await serviceClient.from("page_sections").insert(sections);
  }

  const navItems = materializeStoreTemplateNavigation(selectedTemplate);
